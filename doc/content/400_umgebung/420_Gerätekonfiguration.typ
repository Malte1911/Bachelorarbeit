...
Sämtliche Einstellungen an beiden Geräten werden mit SENTRON Powerconfig vorgenommen, das nach RB-01 in der Desktop-Variante zum Einsatz kommt. Das Werkzeug erreicht das Powercenter wahlweise vor Ort über #acro("BLE") oder im Netz über dessen Ethernet-Schnittstelle, auf der das Gerät die mit #acro("TLS") gesicherte #acro("REST")-#acro("API") als Standardweg der Inbetriebnahmesoftware bereitstellt (siehe @sec:powercenter_schnittstellen). Am Testaufbau wird ausschließlich der Weg über das Netz genutzt, der Weg über #acro("BLE") und mit ihm die mobile Anwendung bleiben aus den in @sec:rb genannten Gründen ungenutzt. Die #acro("REST")-#acro("API") ist dabei allein der Übertragungsweg des Werkzeugs und wird nicht selbst bedient. Da die Endgeräte nach @sec:ecpd_konnektivitaet für ein übergeordnetes System unmittelbar unerreichbar sind, reicht das Powercenter die Parametrierung eines #acro("ECPD") über die Funkstrecke weiter, weshalb beide Geräte über diesen einen Zugang eingerichtet werden.
...
Eine eigene Festlegung betrifft den Nennstrom. Der Aufbau verwendet nach @sec:testaufbau die $16space.thin"A"$-Variante des #acro("ECPD"), deren Nennstrom sich nach @sec:ecpd_geraet zusätzlich parametrieren lässt und am Testaufbau auf $10space.thin"A"$ herabgesetzt ist. Der Grund liegt bei T-06. Die verfügbare Last erreicht rund $15class("normal", ",")7space.thin"A"$ und bliebe am unveränderten Nennstrom unterhalb der Auslöseschwelle, sodass sich weder eine Auslösung noch der zugehörige Alarm in vertretbarer Zeit beobachten ließe. Mit dem herabgesetzten Wert entspricht die Last dem 1,57-fachen des eingestellten Nennstroms. Ergänzend ist der Zeitraum der verzögerten Auslösung auf den kleinsten einstellbaren Wert gesetzt, damit sich der Vorgang innerhalb einer beaufsichtigten Zeitspanne beobachten lässt. Die Grenzwerte der Alarme bleiben demgegenüber unverändert. Sie sind nach @sec:datenpunkte in Prozent des Nennstroms angegeben und wandern mit dessen Herabsetzung ohne eigenen Eingriff mit, sodass ein einzelner Grenzwert für T-06 nicht anzutasten war. Dieselbe Bezugsgröße gilt für sämtliche Stromgrenzwerte des Geräts, die sich am Testaufbau folglich alle auf diese $10space.thin"A"$ beziehen.
