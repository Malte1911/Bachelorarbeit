...
The selection of data points forms the core of the work. A line consisting of one Powercenter and one end device comprises 363 entries of the register map, which can neither be displayed meaningfully nor polled at the required rate, and each further end device adds 152. Seven selection criteria reduce this scope to 55 registers read and thereby lower the polling load by roughly 85 percent. The result consists of machine-readable type descriptions for both device types, a listing that records register, format and justification for every data point, and a document addressed to installers and operators.
