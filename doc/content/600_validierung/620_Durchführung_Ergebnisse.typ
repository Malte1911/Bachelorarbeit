...
Damit ist FA-01 für beide Gerätetypen belegt. Die Prüfung des Powercenters endet an dieser Stelle, wie @sec:pruefablauf festhält. Beobachtet wurde dabei, dass die abgebildeten Werte in der vorgesehenen Form ankommen, ohne dass dies einen eigenen Testfall begründet.
...
Hervorgerufen wurde die Änderung über SENTRON Powerconfig und an einem Stammdatum, was für die Aussagekraft der Beobachtung günstig ist. Die gestaffelten Intervalle gelten für die Werte, die das Endgerät von sich aus überträgt, für Messwerte und Zustände. Die Stammdaten gehören nicht dazu, da sie sich nach @sec:datenpunkte im Betrieb nicht ändern. Zugleich läuft der schreibende Zugriff nach @sec:geraetekonfiguration über dasselbe Powercenter, über das auch Desigo CC liest. Die Änderung erschien kurz nach dem Schreibvorgang in der Bedienoberfläche, ohne dass die Zeitspanne gestoppt worden wäre. Damit trifft die Beobachtung genau den Fall, den das erwartete Ergebnis nach @tab:testfaelle beschreibt, denn dieses rechnet die Frist ausdrücklich von dem Zeitpunkt an, zu dem der Wert am Powercenter bereitsteht, und nicht von der Änderung am Endgerät an. Den belastbaren Nachweis des eingestellten Intervalls führt ohnehin die Aufzeichnung des Telegrammverkehrs.
...
T-05 ist erfüllt. Beschriftung, Einheit, Skalierung und Vorzeichen stimmen für die geprüften Datenpunkte mit der Registerkarte und dem angezeigten Wert überein, mehrwortige Zeichenketten werden in der richtigen Wortreihenfolge gelesen. Die geprüften Datenpunkte sind dabei nach @sec:uebernahme in beiden Ansichten der Bedienoberfläche erreichbar, sodass die Zuordnung zu einer Gruppe die Anzeige gliedert, ohne einen Wert unauffindbar zu machen.


==== Alarme und Meldungsverhalten

T-06 ist nicht erfüllt. Die Überlast nach @sec:testaufbau lässt das #acro("ECPD") nach rund $30space.thin"s"$ in den Standby-Zustand übergehen und setzt im Sammelregister das Bit 5, den Alarm 1 Überstrom nach @tab:apx_ecpd_alarme. Weitere Bits bleiben ungesetzt, insbesondere das Bit 13 für den ausgelösten Schalter, denn im Standby bleibt der mechanische Trennkontakt nach @sec:ecpd_geraet geschlossen und allein der Leistungshalbleiter sperrt. Der Schalterstatus in Register 3110 führt dabei den Wert 6 für den störungsbedingten Standby und nicht den Wert 5 für den planmäßigen. Damit leistet dieses Register genau die Unterscheidung, für die es nach @sec:datenpunkte aufgenommen wurde, und der störungsbedingte Zustand ist auch dann als eigener Datenpunkt erkennbar, wenn das Sammelregister keinen ausgelösten Schalter meldet. Der Zustand erreicht Desigo CC, jedoch nicht als einzelner, benannter Datenpunkt. Da sich das Sammelregister nach @sec:umsetzung weder über einen Subindex noch über einen #acro("BLOB") in eine Typbeschreibung überführen lässt, die Desigo CC annimmt, ist es als eine vorzeichenlose Ganzzahl abgebildet. Der ausgelöste Zustand ist damit als Änderung dieses einen Wertes beobachtbar und über @tab:apx_ecpd_alarme einer Meldung zuzuordnen, steht in Desigo CC aber nicht als eigener Zustand zur Auswertung bereit. Das erwartete Ergebnis aus @tab:testfaelle wird somit nicht erreicht.

T-07 ist nicht durchführbar. Der Testfall setzt eine eingerichtete Alarmkonfiguration voraus, die sich aus dem Datenmodell heraus nicht anlegen lässt und die auf einen Datenpunkt je Zustand angewiesen wäre. Solange die Zerlegung des Sammelregisters nicht gelingt, fehlt dieser Konfiguration die Grundlage.
...
#figure(
  table(
    ...
    [T-05], [erfüllt],
    [Beschriftung, Einheit, Skalierung und Wortreihenfolge geprüft, keine Abweichung],
