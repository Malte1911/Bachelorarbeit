...
Das eigentliche Unterscheidungsmerkmal liegt jedoch in der Ausführung des Schaltpfads. Das Gerät verfügt neben dem mechanischen Trennkontakt über einen Leistungshalbleiter und kennt dadurch einen dritten Betriebszustand, der als Standby bezeichnet wird @src:sentronsystemhandbuch. In diesem Zustand ist der Halbleiter hochohmig, der Stromkreis damit nicht leitend, ohne dass der mechanische Kontakt geöffnet wurde @src:sentronsystemhandbuch. Ein Rückschalten in den leitenden Zustand ist damit ohne Eingriff vor Ort möglich, etwa um Standby-Verbraucher gezielt abzuschalten oder nach einer Überlastauslösung wieder zuzuschalten @src:sentronsystemhandbuch. Halbleiterbasierte Schaltpfade kommen gegenüber dem rein elektromechanischen Kontakt ohne bewegte Teile und ohne Lichtbogen aus, erreichen dadurch eine erheblich höhere Zahl an Schaltspielen und schalten um Größenordnungen schneller. Sie erfordern jedoch eine Beherrschung der Verlustleistung im leitenden Zustand und der Überspannungen, die beim Abschalten aus der Induktivität des Netzes entstehen, und sind deshalb Gegenstand aktueller Forschung @src:rodrigues2021. Wird das Gerät hingegen per Befehl in den Zustand OFF ausgelöst, öffnet der mechanische Trennkontakt. Ein Fernschalten ist danach nicht mehr möglich, das Gerät muss vor Ort mechanisch wieder eingeschaltet werden @src:sentronsystemhandbuch.

Aus der elektronischen Ausführung folgt eine zweite Eigenschaft, die klassische Schutzschaltgeräte nicht besitzen. Ein erheblicher Teil des Geräteverhaltens ist parametrierbar @src:sentronsystemhandbuch. Die Empfindlichkeit der Fehlerstromauslösung lässt sich von $22,5space.thin"mA"$ auf $18,0space.thin"mA"$ oder $27,0space.thin"mA"$ ändern, und je Auslöseursache ist einstellbar, wie sich das Gerät nach der Auslösung verhält, bis hin zum automatischen Wiedereinschalten über die #acro("ARD")-Funktion @src:sentronsystemhandbuch. Da diese Einstellungen die Schutzwirkung selbst betreffen, sind sie als geschützte Parameter ausgeführt. Ihre Änderung setzt voraus, dass der Bereich freigegeben und anschließend innerhalb einer vorgegebenen Zeit die Taste am Gerät gedrückt wird @src:sentronsystemhandbuch. Alternativ lässt sich das Fernschalten über eine Benutzerrolle mit Vollzugriff freigeben @src:sentronsystemhandbuch. Jede Änderung geschützter Parameter wird im Gerät protokolliert, und im Auslieferungszustand sind die betreffenden Funktionen deaktiviert @src:sentronsystemhandbuch.

Schließlich führt das Gerät einen zyklischen Selbsttest durch, der interne Funktionen prüft und das Gerät im Fehlerfall in einen sicheren Zustand schaltet @src:sentronsystemhandbuch. Ergänzend lässt sich ein Test der Fehlerstromschutzfunktion anstoßen, bei dem ein Testsignal in den Messkreis eingebracht und dessen Erkennung geprüft wird @src:sentronsystemhandbuch. Die Ergebnisse werden mit Zeitstempel im Gerät abgelegt, das bis zu 60 Testeinträge, bis zu 126 Meldungen und bis zu 100 Auslösungen speichert @src:sentronsystemhandbuch @src:sentronregistermap.
