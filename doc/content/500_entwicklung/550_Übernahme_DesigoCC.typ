...
Innerhalb des Projekts wird anschließend ein Treiber für das Subsystemnetzwerk angelegt, der die Modbus-Kommunikation verwaltet. An ihm wird das Abfrageintervall auf eine Sekunde gesetzt. Da es nach @sec:konzept für sämtliche Datenpunkte aller angebundenen Geräte gilt, bestimmt der am schnellsten benötigte Wert die Einstellung für alle übrigen, und das ist mit dem Schalterzustand ein Datenpunkt der Gruppe des Live-Zustands. Zugleich ist eine Sekunde die Untergrenze, die das Systemhandbuch für die Abfrage eines Geräts empfiehlt @src:sentronsystemhandbuch, sodass sich beide Vorgaben an derselben Stelle treffen.
