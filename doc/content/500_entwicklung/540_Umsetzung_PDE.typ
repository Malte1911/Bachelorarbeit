...
Der in @sec:geraetekonfiguration beschriebene Versatz von eins zwischen der Zählweise der Registerkarte und der Adressierung im Telegramm wird nicht in jeder einzelnen Eigenschaft nachgeführt, sondern einmalig über das Merkmal des Adressversatzes gesetzt, das das Werkzeug zur Basisadresse addiert @src:pdemanual. Jede Eigenschaft der Typbeschreibung trägt dadurch dieselbe Registernummer wie die Registerkarte @src:sentronregistermap und wie die Aufstellung in #ref(<apx:datenpunkte_ecpd>, supplement: [Anhang]), sodass sich jeder Datenpunkt ohne Umrechnung zurückverfolgen lässt. Für die nach NFA-01 geforderte Dokumentation ist das die Voraussetzung dafür, dass eine spätere Änderung an der richtigen Stelle ansetzt.
