...
Das erste Lösungskonzept in @sec:konzept legt das Gerüst der Lösung fest. Eine einzige Typbeschreibung bildet alle Geräte desselben Typs ab und wird über den Unit Identifier je physischem Gerät instanziiert, Powercenter und #acro("ECPD") erhalten getrennte Objekttypen, und die Parametrierung der Geräte verbleibt bei SENTRON Powerconfig. Womit dieses Gerüst gefüllt wird, bleibt darin offen. Die Analyse des Registerraums in @sec:registerraum weist rund 5.200 Datenpunkte je Strang aus, von denen nur ein Teil in das Modell gehört. Welcher Teil dies ist, aus welchem Grund und in welcher Form er in der Typbeschreibung erscheint, ist Gegenstand dieses Kapitels.

@sec:auswahlkriterien benennt die Kriterien der Auswahl und führt sie auf die Anwendungsfälle und die Eigenschaften des Registerraums zurück, bevor sie angewandt werden. @sec:datenpunkte wendet sie auf beide Gerätetypen an und stellt die getroffene Auswahl der vollständigen Abbildung gegenüber. Aus dem Ergebnis entsteht in @sec:umsetzung die Typbeschreibung im #acro("PDE") mit der Zuordnung zu Datentypen, Transformationen und Registeradressen. @sec:uebernahme beschreibt den Import in Desigo CC und das Anlegen der Geräteinstanzen, @sec:modelldoku die begleitende Dokumentation, ohne die das Entwicklungsergebnis nach NFA-01 und NFA-02 unvollständig bliebe. Der Schwerpunkt liegt durchgehend auf dem Objektmodell des #acro("ECPD"), da ein eigenes Objektmodell für das Powercenter nach @sec:konzept nicht gefordert ist.
