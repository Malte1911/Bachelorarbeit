...
Hinzu kommt die Skalierung innerhalb einer Anlage. Aus einer Typbeschreibung entstehen beliebig viele Instanzen, denen allein ihre Kommunikationsparameter mitzugeben sind, sodass der Aufwand mit der Zahl der Geräte nicht linear wächst. Die in @sec:datenpunkte begründete Reduktion des Umfangs wirkt in dieselbe Richtung, da sie den abgebildeten Registerraum eines voll bestückten Strangs um rund drei Viertel verkleinert und damit den Betrieb mehrerer Stränge an einem Server erleichtert.

Beide Punkte stehen unter zwei Vorbehalten. Solange die Alarmierung nach @sec:praxistauglichkeit je Anlage von Hand einzurichten ist, bleibt ein Teil der Handarbeit bestehen, den die Vorlage gerade abnehmen sollte. Und die Wiederverwendbarkeit ist nach @sec:anforderungsabgleich an einem einzelnen #acro("ECPD") belegt, nicht an einem vollständigen Strang unter Betriebsbedingungen. Beide Nutzenaussagen sind darüber hinaus als erwarteter und nicht als gemessener Nutzen zu lesen, denn weder der eingesparte Aufwand noch die Wirkung der geringeren Abfragelast auf die Zahl der an einem Server betreibbaren Stränge ist in dieser Arbeit erhoben worden.
