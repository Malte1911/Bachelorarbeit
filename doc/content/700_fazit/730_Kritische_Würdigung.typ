...
Eine letzte Einschränkung betrifft nicht die Lösung selbst, sondern den Anforderungskatalog, an dem sie gemessen wird. Die Rückverfolgung in @tab:apx_rueckverfolgung zeigt, dass zwei Anwendungsfälle ohne eigene Anforderung geblieben sind: UC-07 mit den Zähler- und Wartungsdaten und UC-09 mit den Stammdaten des #acro("ECPD"). Beide sind in der Auswahl der Datenpunkte enthalten, weil sie sich unmittelbar aus den Anwendungsfällen ergeben. Verlangt hat sie dagegen keine Anforderung, da FA-03 nach @sec:fa bewusst auf die Messwerte begrenzt ist. Die Auswahl geht an dieser Stelle also über den Katalog hinaus. Für das Ergebnis bleibt das folgenlos, denn T-05 erstreckt sich auf jeden abgebildeten Datenpunkt und nicht allein auf die Messwerte. Eine Fortschreibung des Katalogs sollte die beiden Anwendungsfälle dennoch mit eigenen Anforderungen unterlegen. FA-03 nachträglich zu erweitern wäre dagegen der falsche Weg, denn dessen Begrenzung auf die Messwerte trägt die in @sec:registerraum begründete Reduktion des Registerraums.
