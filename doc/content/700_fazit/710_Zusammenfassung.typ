...
Das Vorgehen folgte dem in @sec:vorgehensmodell gewählten V-Modell. Aus der System- und Stakeholderanalyse gingen zehn Anwendungsfälle hervor, aus diesen 15 Anforderungen mit ihren Randbedingungen und daraus 14 Testfälle, die den aufsteigenden Ast des V-Modells bilden. Jede Festlegung des Modells ist auf diesem Weg bis zu einer Tätigkeit einer benannten Nutzergruppe zurückführbar.

Den Kern der Entwicklung bildet die Auswahl der Datenpunkte. Ein voll bestückter Strang aus einem Powercenter und 24 Endgeräten umfasst nach @sec:registerraum 3859 Einträge der Registerkarte, die weder sinnvoll darstellbar noch im geforderten Takt abfragbar sind. Sieben Auswahlkriterien reduzieren diesen Umfang auf 904 gelesene Register und damit den abgebildeten Registerraum um rund 77 Prozent, wobei jede Ausschlussgruppe ebenso begründet ist wie jede Aufnahme (siehe @sec:datenpunkte). Je #acro("ECPD") verbleiben 37 der 152 Einträge. Die Umsetzung im #acro("PDE") ergänzte dazu Benennung, Gruppenzuordnung, Adressierung und Datentypen, und die Übernahme in Desigo CC bestätigte diesen Weg an einer Werkzeugkette, deren Zielplattform der #acro("PDE") nach @sec:pde_ziel gar nicht als Zielapplikation führt.

Das Ergebnis besteht aus drei Teilen. Die Typbeschreibungen des #acro("ECPD") und des Powercenters als #acro("JSON")-Dateien bilden die maschinenlesbare Abbildung, die Aufstellung in #ref(<apx:datenpunkte_ecpd>, supplement: [Anhang]) hält zu jedem Datenpunkt Register, Format und Begründung fest, und die Unterlage nach @sec:modelldoku beschreibt Voraussetzungen, Arbeitsschritte und Grenzen für die drei Adressatenkreise.

Die Validierung am Testaufbau weist zehn der 15 Anforderungen als erfüllt aus, drei als teilweise und zwei als nicht erfüllt (siehe @sec:anforderungsabgleich). Erfüllt ist durchweg, was die Abbildung zwischen Registerraum und Objektmodell betrifft. Offen bleibt die Alarmierung, da sich das Sammelregister mit der eingesetzten Werkzeugkette nicht in einzelne Meldungen zerlegen lässt. Über den Abgleich hinaus hat die Prüfung Eigenschaften der Geräte und der Plattform belegt, die kein Datenmodell beheben könnte, darunter die nur über SENTRON Powerconfig zu setzende Freigabe des Fernschaltens, die ab Werk abgeschalteten Alarmbits und das nur geräteübergreifend einstellbare Abfrageintervall (siehe @sec:befunde).
