...
Am nächsten liegt die nach Verwendungszweck abgestufte Abfrage. Ließe sich das Intervall entgegen @sec:befunde je Datenpunktgruppe einstellen, wäre die Abfragelast eines vollen Strangs über die Auswahl der Datenpunkte hinaus zu senken. Die dafür nötige Einstufung liegt bereits vor: Jeder Datenpunkt trägt nach @sec:datenpunkte in der Arbeitsmappe des Anforderungskatalogs einen Vermerk zur erforderlichen Aktualität, der dort ausdrücklich als Vorgabe für die Projektierung und eine spätere Weiterentwicklung ausgewiesen ist.

Die Typbeschreibung des Powercenters lässt sich abrunden. Sie ist nach @sec:umsetzung bewusst schmal gehalten und trägt die abgebildeten Werte nach @sec:uebernahme in der vorgesehenen Form, ist über Import und Instanzbildung hinaus jedoch nicht mit derselben Tiefe geprüft worden wie die des #acro("ECPD"). Der Zeit- und Synchronisationsstatus ist darin zudem ausgespart geblieben. Seine Aufnahme wäre eine kleine Ergänzung, deren Nutzen allerdings an derselben Alarmierung hängt, denn eine abweichende Uhr des Datentransceivers entwertet die Zeitstempel des Strangs und fällt erst auf, wenn sie gemeldet wird.
