...
#figure(
  table(
    ...
    [Verfolgt dasselbe Interesse wie der selbst entwickelnde Endkunde, allerdings mit dem Ziel einer fertigen Lösung, die einem Projekt ohne weitere Entwicklungsarbeit beigegeben werden kann.],
    [hoch],
  ),
...
Drei Wege scheiden ohne nähere Bewertung aus. Der unmittelbare Zugriff auf das Endgerät (W1) ist technisch versperrt, da die Schutzschaltgeräte keine Modbus-Schnittstelle besitzen und ausschließlich über die Funkstrecke mit dem Powercenter kommunizieren @src:sentronsystemhandbuch. Die #acro("BLE")-Schnittstelle des Powercenters (W2) ist als örtlicher Zugang ausgelegt, unterstützt nur eine aktive Verbindung und schaltet sich nach $180space.thin"s"$ ohne Nutzung ab @src:sentronsystemhandbuch, womit sie für den zyklischen Dauerbetrieb ausfällt. Die Cloud-Anbindung über #acro("MQTT") (W5) steht allein am Powercenter 2000 zur Verfügung @src:sentronsystemhandbuch, während der Testaufbau ein Powercenter 1100 vorsieht. Sie verlangt zudem eine Rolle, die Desigo CC auf der Feldebene nicht einnimmt, da die Plattform die Geräte eines Netzwerks über einen eigenen Treiber zyklisch ausliest, das Powercenter seine Werte über #acro("MQTT") hingegen von sich aus an einen Cloud-Dienst sendet @src:sentronsystemhandbuch. Ein Erweiterungsmodul, das diese Werte entgegennähme, führt die Engineering-Dokumentation nicht @src:desigoccenghelp.
...
/* Claude: Drei Punkte aus der Durchsicht sind hier abgearbeitet.
   ...
   3. W6 ist mit @src:sentronsoftwareguide belegt und sprachlich getrennt.
      ...
      Powermanager stellt northbound OPC bereit, das Powercenter 3000
      ausschliesslich MQTT. Der Satz, man koenne "dieses System ueber OPC an
      Desigo CC koppeln", galt also nicht fuer beide Produkte.
   4. Der Ausschluss von W5 ist am 11.09.2026 nachgeprueft und um die
      Rollenfrage erweitert. Geprueft wurden die Inhaltsverzeichnisse und der
      Suchindex der Engineering Help in den Staenden V5.1, V6 und V7. MQTT
      kommt dort kein einziges Mal vor, waehrend jedes unterstuetzte
      Feldprotokoll ein eigenes Treiberkapitel fuehrt (BACnet, OPC, Modbus,
      SNMP, S7, KNX, IEC 61850). Die im Netz auffindbaren Siemens-Dokumente zu
      MQTT gehoeren zu anderen Erzeugnissen, zu Building X
      (Dokument-ID A6V14054665) und zu Desigo Optic (A6V11890766). Der
      Building X Connector in Desigo CC V7 laedt Geraete und Datenpunkte aus
      der Plattform in die Cloud hoch und fuehrt damit in die
      Gegenrichtung. */
...
Für den Zuschnitt der Lösung ist dabei entscheidend, dass die Typbeschreibung des #acro("PDE") die Adressierung bereits mitführt (siehe @tab:pde_schritte), Objektmodell und Adressbelegung damit in derselben Datei liegen. Die Lösung dieser Arbeit besteht folglich aus einem einzigen zu entwickelnden Artefakt und nicht aus einem Objektmodell mit einem daneben zu pflegenden Regelwerk, wie es der allgemeine Importweg der Engineering-Dokumentation vorsieht @src:desigoccenghelp.
...
#figure(
  table(
    ...
    [Abfrageintervall],
    [Das Intervall, in dem der Treiber die Geräte abfragt, wird an der Treiberkonfiguration eingestellt. Daneben kennt die Plattform benannte Abfragegruppen mit eigenem Intervall, die einem lesenden Datenpunkt zugeordnet werden und über den tabellarischen Geräteimport in das Projekt gelangen. Ohne eine solche Zuordnung folgt ein Datenpunkt dem Intervall des übergeordneten Netzknotens. Die vom #acro("PDE") erzeugte Typbeschreibung führt keine Abfragegruppe mit, sodass auf dem hier beschrittenen Weg für sämtliche Datenpunkte aller Geräte eines Treibers dasselbe Intervall gilt und sich die Abfragelast allein über die Zahl der abgebildeten Datenpunkte und über dieses eine Intervall steuern lässt.],
