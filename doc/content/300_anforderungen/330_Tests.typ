...
*T-04 prüft die Plausibilität, nicht die Messgenauigkeit.* Die Genauigkeit der vom #acro("ECPD") erfassten Größen ist mit den verfügbaren Mitteln nicht nachweisbar und wäre auch keine Eigenschaft des Datenmodells, sondern des Geräts. Geprüft wird daher, ob der richtige Wert an der richtigen Stelle ankommt: ob die übertragenen Werte dem tatsächlichen Betriebszustand entsprechen, der aufgeschalteten Last folgen und keine Verwechslung von Kanälen, Vorzeichen oder Skalierungen erkennen lassen. Eine Abweichung gegenüber der Referenz innerhalb der Gerätetoleranz ist unerheblich; ein Wert, der der Last nicht folgt, ist es nicht.
